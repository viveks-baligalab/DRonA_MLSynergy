...
def prepare_combinations_file(metadata_loc,N,tr_times):
    ...
    d_c_t_ps = ["%s_%0.2f_%0.0f"%names for names, group in \
             metadata.groupby(["Drug","xMIC50","Treatment time"])\
...
def make_sample_dict(metadata):
    ret_dict = {}
    for name, group in metadata.groupby(["Drug","xMIC50","Treatment time"]):
...
def new_mlsyn(trained_classifier,data,sd,combination,Tr,ND):
...
def get_mlsyn_score(trained_classifier,data,metadata,combination_metadata):
    ...
    for ni, (n, row) in enumerate(metacomb.iterrows()):
        ...
        if row.NumbDrugs in [1,2]:
            d1,d2,d12_e,d12_o = new_mlsyn(trained_classifier,data,sd,n,trt,2)
            mlsyn_sc = d12_e/d12_o
            expected, observed = d12_e, d12_o
        elif row.NumbDrugs >= 3:
            d1,d2,d3,d123_e,d123_o = new_mlsyn(trained_classifier,data,sd,n,trt,3)
...
def main(argv):
    #### help strings
    help_str1 = "MLSynergy.py --help --command <> --[conditional]classifier <> --[conditional]metadata <> --[conditional]combinaitons --[conditional]data <> --[conditional]ref_ids <> --output [OPTIONS]"
    options_str2 = "--command or -c: <str> Make_combinations or Score_combinations\n\
--[conditional]classifier or -clf: <str> If command = Score_combinations; file location of the classifier should be provided\n\
--meatadata or -m: <str> If command = Make_combinations; metadata file of samples with information on drug treatment (csv format)\n\
--[conditional]combinations or -com: <str> If command = Score_combinations; combinations file generated by Make_combination command (csv format)\n\
...
--[conditional]ref_ids or -r: <str> If command = Score_combinations; file (text format) with RefSeq IDs of the organism in study\n\
--[conditional]output or -o: <str> If command = Make_combinations; file to write the combinations as (csv format) in\n\
OPTIONS:\n\
--plot_chord or -plt: <bool> True or False for plotting predictions of two drug interaction; default: False\n\
--comb_number or -N: <comma sperated> If command = Make_combinations; Number of drug to make the combinations with; default = 2,3\n\
--treatment_durations or -td: <comma sperated> If command = Make_combinations; Treatment duration (in hours) to concider to make the combinations with; default = 72\n\
--rank_normalize or -rn: <bool> True or False for rank normalisation; default: True\n\
--clean_data or -cln: <bool> True or False for removing transcritomes with large number of missing data; default: False\n\
--[conditional]norm_method or -norm: <str> If rank_normalize is True, select from [average,min,max,dense,ordinal]; default: min"
    #### define classical python input
    command,classifier,metadata,combinations,data,ref_ids,output = "","","","","","",""
    clean_data,normalize,norm_method,N,td,plt = "False","True","min",[2,3],[72],"False"
    
    try:
        opts, args = getopt.getopt(argv,"h:c:clf:m:com:d:r:o:rn:cld:nm:N:td:plt",\
                                   ["help","command=","classifier=","metadata=","combinations=","data=","ref_ids=","output=",\
    ...
    for opt, arg in opts:
        if opt in ("-h","--help"):
            ...
            command = arg
        elif opt in ("-clf","--classifier"):
            classifier = arg
        elif opt in ("-m","--metadata"):
            metadata = arg
        elif opt in ("-com","--combinations"):
            combinations = arg
        elif opt in ("-d","--data"):
            data = arg
        elif opt in ("-r","--ref_ids"):
            ref_ids = arg
        elif opt in ("-o","--output"):
            output = arg
        elif opt in ("-rn","--rank_normalize"):
            normalize = arg
        elif opt in ("-cln","--clean_data"):
            clean_data = arg   
        elif opt in ("-nm","--norm_method"):
            norm_method = arg
        elif opt in ("-N","--comb_number"):
            N = [int(i) for i in arg.split(",")]
        elif opt in ("-td","--treatment_durations"):
            td = [int(i) for i in arg.split(",")]
        elif opt in ("-plt","--plot_chord"):
            plt=arg
