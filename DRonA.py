...
def return_scores2(classifier,data,metadata):
    ...
    for n, row in meta_data.iterrows():
        try:
            meta_data.at[n,"CVS"] = scores[n]
            meta_data.at[n,"ref_CVS"] = max(scores.values())
        except:
            pass
    return meta_data
...
def main(argv):
    #### help strings
    help_str1 = "DRonA.py --help --command <> --classifier[conditional] <> --metadata <> --data <> --ref_ids <> --output <> [OPTIONS]"
    options_str2 = "--command or -c: <str> Train or Score\n\
--[conditional]classifier or -clf: If command = Score; file location of the classifier should be provided\n\
--meatadata or -m: <str> Metadata file (csv format)\n\
--data or -d: <str> Datafile (csv format) with gene expession data\n\
--ref_ids or -r: <str> File (text format) with RefSeq IDs of the organism in study\n\
--output or -o: Output folder to write the classifier or results in\n\
OPTIONS:\n\
...
--cb or -cb: <str> Label of row in metadata file having data labels for training and testing; default: Sample_class\n\
--sp or -sp: <str> Label for iniital seed data; default: Manually IDd viable\n\
--sn or -sn: <str> Label for test data; default: Manually IDd non-viable\n\
--k or -k: <str> Kernel [linear,poly,rbf,sigmoid,precomputed]; default: linear\n\
--[conditional]dg or -dg: <int: 1 to inf> if kernel is poly; default: 1\n\
--ga or -ga: <str> Gamma [scale,auto]; default: auto\n\
--gn or -gn: <float: 0.0 to 1.0> Gain; default: 0.9\n\
--to or -to: <float: 0.0 to 1.0> Tolerance; default: 0.9\n\
--th or -th: <float: 0.0 to 1.0> Threshold; default: 0.85\n\
--[conditional]norm_method or -norm: <str> If rank_normalize is True, select from [average,min,max,dense,ordinal]; default: min"
    ...
    command,classifier,metadata,data,ref_ids,output = "","","","","",""
    clean_data,normalize,norm_method,classify_by,seed_positives,seed_negatives = "False","True","min","Sample_class","Manually IDd viable","Manually IDd non-viable"
    kernel,degree,gamma,gain,tolerance,threshold = "linear",1,"auto",0.9,0.9,0.85
    try:
        opts, args = getopt.getopt(argv,"h:c:clf:m:d:r:o:rn:cld:cb:sp:sn:k:dg:ga:gn:to:th:nm",\
    ...
    #### run commands
    if command and metadata and data and ref_ids and output:
        ### Read data
